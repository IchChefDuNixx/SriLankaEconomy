...
def plot_panel1(data: dict[str, dict[str, pd.DataFrame]]) -> None:
    """
    Create a multi-panel visualization of Sri Lanka's economic and social indicators.

    Args:
        data: Nested dictionary containing DataFrames for different indicators
    """
    ...
    # Filter datasets
    datasets = {
        'Inflation': data['inflation']['sl']['Inflation'],
        'GDP': data['GDP']['sl']['GDP per capita (current US$)'],
        'Happiness': data['happiness']['sl']['Happiness score'],
        'Tourism': data['tourism']['sl']['tourists arrived']
    }

    filtered_data = {
        name: values.reindex(visible_years)
        for name, values in datasets.items()
    }

    # Plot configuration
    axis_configs = {
        'Inflation': {'range': [0, 55], 'title': "Inflation (%)"},
        'GDP': {'range': [0, 5100], 'title': "GDP per capita"},
        'Happiness': {'range': [0, 9], 'title': "Happiness Score"},
        'Tourism': {'range': [0, 2.5e6], 'title': "Yearly Tourist Arrivals"}
    }
    ...
    for i, (name, data) in enumerate(filtered_data.items(), start=1):
        fig.add_trace(
            go.Scatter(
                x=data,
                xaxis=f"x{i}",
    ...
    xaxis_layouts = {}
    for i, (name, config) in enumerate(axis_configs.items(), start=1):
        domain_start = (i - 1) * 0.25
        xaxis_layouts[f'xaxis{i}'] = dict(
            domain=[domain_start, domain_start + 0.25],
            range=config['range'],
            title=config['title'],
            side="top"
        )
